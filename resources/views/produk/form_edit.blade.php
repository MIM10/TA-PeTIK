...
@section('content')
    ...
    {{-- Ini Konten Form Edit produk --}}
    {{-- Panggil master data produk, penerbit dan kategori untuk ditampilkan di element form edit produk --}}
    @php
        $rs1 = App\Models\Produk::all();
    @endphp
    @foreach ($data as $rs)
        <form method="POST" enctype="multipart/form-data" action="{{ route('produk.update', $rs->id) }}">
    @endforeach
    @csrf {{-- security untuk menghindari dari serangan pada saat input form --}}
    @method('put') {{-- method put digunakan untuk meletakkan data yang akan diubah disetiap element form edit produk --}}
